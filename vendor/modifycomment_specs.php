...
$comment_id = strtolower($_POST['comment_id']);
$h_remaining = strtolower($_POST['h_remaining']);
$pvm_remaining = strtolower($_POST['pvm_remaining']);
$readiness = strtolower($_POST['readiness']);

$meta = mysqli_query($db, "UPDATE `comments` SET `h_remaining`='$h_remaining',`pvm_remaining`='$pvm_remaining',`readiness`='$readiness' WHERE `id`='$comment_id'");
...
$meta = mysqli_query($db, "INSERT INTO `changed__comments_interactive` (`commentid`, `projectid`,`room`,`name`,`x_y`, `content`, `attachments`, `comment_from`, `comment_to`, `urgency`, `creation_time`, `ending_time`, `aihe`, `deadline`, `answer_to`,`time_estimate`,`status`,`tja`,`tta`,`h_remaining`,`pvm_remaining`,`readiness`,`myyja_lupaus`) VALUES ('$c_ch_n[0]', '$pr_name', '$c_ch_n[2]', '$c_ch_n[3]', '$c_ch_n[4]', '', '', '', '', '$c_ch_n[9]',  '', '', '', '$c_ch_n[13]', '', '$c_ch_n[15]','','','','$h_remaining','$pvm_remaining','$readiness','') ON DUPLICATE KEY UPDATE `h_remaining`='$h_remaining',`pvm_remaining`='$pvm_remaining',`readiness`='$readiness';");
