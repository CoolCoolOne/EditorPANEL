...
$vko = strtolower($_POST['vko_arvio']);

$meta = mysqli_query($db, "UPDATE `comments` SET `tja`='$vko',`tta`='$vko' WHERE `id`='$comment_id'");
print_r($meta);

// $comment_id = "com01ab16f48b263";
// $vko = strtolower("51");
...
$meta = mysqli_query($db, "INSERT INTO `changed__comments_interactive` (`commentid`, `projectid`,`room`,`name`,`x_y`, `content`, `attachments`, `comment_from`, `comment_to`, `urgency`, `creation_time`, `ending_time`, `aihe`, `deadline`, `answer_to`,`time_estimate`,`status`,`tja`,`tta`,`h_remaining`,`pvm_remaining`,`readiness`,`myyja_lupaus`) VALUES ('$c_ch_n[0]', '$pr_name', '$c_ch_n[2]', '$c_ch_n[3]', '$c_ch_n[4]', '', '', '', '', '$c_ch_n[9]',  '', '', '', '$c_ch_n[13]', '', '$c_ch_n[15]','','$vko','$vko','$c_ch_n[19]','','','') ON DUPLICATE KEY UPDATE `tja`='$vko',`tta`='$vko';");
